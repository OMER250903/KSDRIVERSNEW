...
// ✅ פונקציה בטוחה לעדכון נהג בלי למחוק שדות קיימים כמו rejected, flagged וכו׳
...
app.post("/edit-driver/:id", ensureLoggedIn, (req, res) => {
  ...
  updateDriver(driverId, {
    ...
    employerPhone,
    employer,
    coordinations: driverData[driverId].coordinations
  });
...
app.get("/driver/:id/coord-gatepass/:index", ensureLoggedIn, (req, res) => {
  ...
  driverData[driverId].coordinations[index] = {
  ...coordination,
...
app.post("/driver/:id/add-coordination", ensureLoggedIn, (req, res) => {
  ...
  updateDriver(driverId, {
    coordinations: [...(driverData[driverId].coordinations || []), newCoordination]
  });
  res.redirect(`/driver/${driverId}`);
});
