...
const loginAttemptsPath = path.join('/data', 'login_attempts.json');
...
function can(roleOrPermissions, action) {
  ...
  return permissions[roleOrPermissions] && permissions[roleOrPermissions].includes(action);
}
...
function saveLoginAttempts() {
    fs.writeFileSync(loginAttemptsPath, JSON.stringify(loginAttempts, null, 2), 'utf8');
}
...
// טעינת נהגים
const filePath = path.join('/data', 'drivers.json');
let driverData = {};
...
app.get('/', ensureLoggedIn, (req, res) => {
    ...
    if (search) {
        results = results.filter(([id, driver]) =>
...
app.post('/add-driver', (req, res) => {
  ...
  const newDriver = {
    ...
    notes: [],
    hasBlock: false,
    image: 'https://cdn.glitch.global/64a24585-7ccf-4cfb-bfc3-67e1b6c37fe4/%D7%90%D7%99%D7%A9%201.png?v=1748175811296',
    driverStatus: false,
    passedAt: null,
    coordinations
  };
...
app.post('/edit-driver/:id', ensureLoggedIn, (req, res) => {
  ...
  const duplicate = Object.entries(driverData).some(([id, driver]) =>
    driver.idNumber === idNumber && id !== driverId
  );
  if (duplicate) {
    return res.render('edit-driver', {
      driverId,
...
const routes = toArray(req.body.route); // ✅ נוספה תמיכה בנתיב
  ...
  driverData[driverId].coordinations = (driverData[driverId].coordinations || []).map(coord => {
...
app.post('/transfer-coordination/:driverId/:coordIndex', ensureLoggedIn, (req, res) => {
  try {
    ...
    saveDrivers();
    console.log(`🔄 העברה: מתיאום אצל ${sourceDriver.name} אל ${targetDriver.name}`);
...
app.post('/upload-coordinations', ensureLoggedIn, upload.single('coordinationsFile'), (req, res) => {
  if (!can(req.session.permissions, 'upload-coordinations')) {
  ...
  parseCSV(fileContent, { columns: true, trim: true }, (err, records) => {
    ...
    const blacklistPath = path.join('/data', 'blacklist.json');
  ...
  idSimilarity(item.idNumber, idNumber) >= 0.9
    ...
    if (blacklistedDrivers.length > 0) {
      ...
      fs.writeFileSync(filePath, JSON.stringify(groupedById, null, 2), 'utf-8');
    ...
    for (const idNumber in groupedById) {
      ...
      const existingEntry = Object.entries(driverData).find(([_, d]) => d.idNumber === idNumber);
...
app.post('/update-yuval', (req, res) => {
  ...
  const yuvalPath = path.join('/data', 'yuval.json');
  let yuvalData = {};
...
res.render('manage-users', { users, permissions: req.session.permissions || [] });
});
...
const usernameToDelete = req.body.username ? req.body.username.trim().toLowerCase() : '';
...
app.get(`/cron/check-flagged-drivers/${SECRET_TOKEN}`, (req, res) => {
  ...
  if (flaggedDrivers.length > 0) {
    const list = flaggedDrivers.map(([id, d]) => `• ${d.name} (${d.idNumber})`).join('\n');
...
app.post('/login', async (req, res) => {
    ...
    if (userAttempt.fails >= 5 && now - userAttempt.lastAttempt < blockDuration) {
        ...
        return res.send('⛔️ חשבונך נחסם זמנית לאחר מספר ניסיונות כושלים. נסה שוב בעוד כמה דקות.');
    }
...
app.post('/edit-password/:username', async (req, res) => {
    ...
    if (!newPassword || newPassword.length < 4) {
        return res.render('edit-password', { username, errorMessage: 'הסיסמה חייבת להכיל לפחות 4 תווים' });
    }

    if (newPassword !== confirmPassword) {
        return res.render('edit-password', { username, errorMessage: 'הסיסמאות אינן תואמות' });
    }
...
app.post('/update-coordination-status/:id/:index', ensureLoggedIn, (req, res) => {
  ...
  if (!driverData[driverId] || !Array.isArray(driverData[driverId].coordinations)) {
...
phone: driver.phone || driver.phoneNumber || '',
  ...
  const cleanDrivers = passedDrivers.filter(d => d && Object.keys(d).length > 0);
  if (cleanDrivers.length === 0) {
    return res.status(400).send('⚠️ אין נתונים זמינים לייצוא.');
  }

  const fields = ['name', 'idNumber', 'phone', 'employer', 'coordinationNumber', 'goodsType', 'donorOrg', 'passedAt'];
...
app.get('/driver/:id/coord-gatepass/:index', ensureLoggedIn, (req, res) => {
  ...
  if (!coordination.passed) {
    ...
    coordination.passedAt = moment.tz(new Date(), 'Asia/Jerusalem').format('HH:mm');
...
app.post('/reject-driver', ensureLoggedIn, (req, res) => {
  ...
  const coord = driver.coordinations.find(c => c.coordinationNumber === coordinationNumber);
...
app.get('/cron/save-statistics', (req, res) => {
  ...
  const statisticsDir = path.join('/data', 'statistics_logs');
...
app.get('/statistics/:date', ensureLoggedIn, (req, res) => {
  ...
  const filePath = path.join('/data', 'statistics_logs', `${selectedDate}.json`);
  const yuvalPath = path.join('/data', 'yuval.json');
  ...
  if (selectedDate) {
    ...
    if (fs.existsSync(filePath)) {
      try {
        const data = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
        data.forEach(driver => {
          const match = !filter || Object.values(driver).join(' ').toLowerCase().includes(filter);
          ...
          if (driver.donorOrg) {
            donorsSummary[driver.donorOrg] = (donorsSummary[driver.donorOrg] || 0) + 1;
          }
        });
      } catch (err) {
    ...
    const statsPath = path.join('/data', 'drivers.json');
    let driverData = {};
    try {
        ...
        driver.coordinations.forEach(coord => {
          ...
          if (coord.passed === true) {
            const match =
              ...
              (coord.goodsType && coord.goodsType.toLowerCase().includes(filter)) ||
              (coord.truckNumber && coord.truckNumber.toLowerCase().includes(filter)) ||
              (coord.donorOrg && coord.donorOrg.toLowerCase().includes(filter)) ||
            ...
            if (coord.donorOrg) {
              donorsSummary[coord.donorOrg] = (donorsSummary[coord.donorOrg] || 0) + 1;
            }
          }
        });
      }
...
const statisticsLogsDir = path.join('/data', 'statistics_logs');

app.get('/search-statistics', ensureLoggedIn, (req, res) => {
  const query = (req.query.query || '').toLowerCase().trim();
  const statsDir = path.join('/data', 'statistics_logs');
...
app.get('/change-password', ensureLoggedIn, (req, res) => {
  ...
  res.render('change-password', { username: req.session.tempUser, errorMessage: null });
});
...
app.post('/change-password', ensureLoggedIn, async (req, res) => {
  ...
  if (!newPassword || newPassword.length < 4) {
    return res.render('change-password', { username, errorMessage: 'הסיסמה חייבת להכיל לפחות 4 תווים' });
  }

  if (newPassword !== confirmPassword) {
    return res.render('change-password', { username, errorMessage: 'הסיסמאות אינן תואמות' });
  }

  // בדיקה - האם הסיסמה היא רק מספרים?
  if (/^\d+$/.test(newPassword)) {
    return res.render('change-password', { username, errorMessage: 'הסיסמה לא יכולה להכיל רק מספרים בלבד' });
  }
  ...
  const isSamePassword = await bcrypt.compare(newPassword.trim(), users[username].passwordHash);
  if (isSamePassword) {
    return res.render('change-password', { username, errorMessage: 'הסיסמה החדשה זהה לסיסמה הנוכחית. אנא בחר סיסמה שונה.' });
  }
